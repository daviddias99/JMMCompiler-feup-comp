...
public class ASTMethod extends SimpleNode {
	...
	public void optimize(Descriptor descriptor, Map<String, Map<String, Integer>> variables) {
		variables = new HashMap<String, Map<String, Integer>>();
		boolean isMain = "main".equals((String)this.jjtGetValue());

		String methodName = (String) this.jjtGetValue();
		List<ASTType> paramTypes;
		if (isMain)
			paramTypes = ((ASTParamList) this.jjtGetChild(0)).getParamTypes();
		else
			paramTypes = ((ASTParamList) this.jjtGetChild(1)).getParamTypes();

		MethodDescriptor methodDescriptor = ((GeneralDescriptor) descriptor).getMethod(methodName, paramTypes);

		for(int i = 0; i < this.jjtGetNumChildren(); i++)
            if(this.jjtGetChild(i) instanceof ASTBody)
				this.jjtGetChild(i).optimize(methodDescriptor, variables);

		if (!isMain)
			this.jjtGetChild(this.jjtGetNumChildren() - 1).optimize(methodDescriptor, variables);
	}
	...
	public void optR(Descriptor descriptor){
		...
		MethodDescriptor methodDescriptor = ((ClassDescriptor) descriptor).getMethod(methodName, paramTypes);

		methodDescriptor.registerAllocationOpt(this);
